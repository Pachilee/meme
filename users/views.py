...
#reverse url before project profile is loaded, if there is no authenticated user logged in 
...
def add_to_list(request, movie_id, movie_name, list_id):
    #404 is searching through userlist, id list 
    user_list = get_object_or_404(UserList, id=list_id, user=request.user) 
    ...
    if ListItem.objects.filter(movie_id=movie_id, list=user_list).exists():
        ...
        message = f"{movie_name} was added {user_list}."
        status = "success"
...
def list_detail(request, list_id):
    ...
    for item in list_items:
        ...
        if response.status_code == 200:
            ...
            movie = response.json()
            movies.append(movie)
    return render (request, 'users/lists/user_list_detail.html', {"movies":movies, "user_list":user_list, "is_owner": request.user.is_authenticated and request.user == user_list.user})
...
def delete_movie(request, movie_id, list_id):
   ...
   #return if method is post otherwise return httpResponseForbidden message
    if request.method == "POST":
        movie.delete()
        list_items = ListItem.objects.filter(list=user_list)
        base_url = "https://api.themoviedb.org/3/movie/"

#fetch movies from listed movies
    movies = []
    for item in list_items:
        movie_id = item.movie_id
        url = f"{base_url}{movie_id}?api_key={API_KEY}"
